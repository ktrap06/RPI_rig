...
"""
stage 2 associative paradigm the animal is only presented with the correct spout;
the first 50 trials: all stim = 1;
trial 51 to 100: all stim =2;
rest are randomly chosen. the mouse will always be rewarded as long as they lick
"""

print("stage 2 version 6")
...
sleep_time = True       #include the 30 second sleep time and 10 second dark frames?
...
sideBias = False #include side bias correction?
...
def main():
    ...
    try:
        ...
        for n in range(1,num_trials+1):
            ...
            #choosing stim 1,2,or 3 (no reward)
            rando = random.uniform(0,1)
            if (rando >= 0) and (rando < 0.4):
                stim = 1
            elif (rando >= 0.4) and (rando < 0.8):
                stim = 2
            elif (rando >= 0.8) and (rando < 0.9):
                stim = 3 #stim 1 (left) without the light
            else:
                stim = 4 #stim 2 (right) without the light
            ...
            #STAGE 2 COMPONENTb
            if 1 >= n: #present stim 1 if the trial number is in the first _ number
                stim =2
            elif 2 >= n > 1:
                stim = 1
            ...
            if stim == 1:
                ...
                visStim(3,ITI)
                ntotal_nogo_left += 1
                print('no stim (1) left presented')
            else:
                visStim(4,ITI)
                ntotal_nogo_right += 1
                print('no stim (2) right presented')
                
            sleep(t_Decide)
            data.append((time.time(),3))
            #GPIO.output([spoutL_pin,spoutR_pin], GPIO.HIGH) #not included because only right answer is presented
            if (stim == 1) or (stim == 3):
                GPIO.output(spoutL_pin, GPIO.HIGH)
            elif (stim == 2) or (stim == 4):
                GPIO.output(spoutR_pin, GPIO.HIGH)
            
            #END OF STAGE2 COMPONENTS
            ...
            if [stim,lickedL,lickedR] in [[1,1,0],[2,0,1]] : #if they lick the correctly associated spout them they get a reward
                success = True
                r=1 #appended if trial was success
            ...
            elif [stim,lickedL,lickedR] in [[3,0,0],[4,0,0]]:
                success = True
                fail = False
                ncorrect_nogo += 1
                t_choice = 0

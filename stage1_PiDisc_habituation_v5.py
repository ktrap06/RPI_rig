...
"""
stage 1 habituation, both spouts are presented the mouse will always be rewarded as long as they lick.
ths spout they dont choose, moves out of the way
"""

print("stage 1 version 5")
...
# Configuration for 60 fps capture
picam2 = Picamera2()
    ...
    try:
        ...
        for n in range(1,num_trials+1):
            ...
            if mpr121[touchL_pin].value or mpr121[touchR_pin].value: #v4
                sensor_error = True
                print('sensor error in Pin {}: pausing trial...'.format(i))
            ...
            if sensor_error == True:
                print('error resolved: continuing trial...')
                sensor_error = False
...
#stage 1 does not have visual stim
           ...
           #STAGE 1 COMPONENTS
            ...
            if stim == 1:
                GPIO.output(solenoidL_pin, GPIO.HIGH)
                sleep(0.1)
                GPIO.output(solenoidL_pin, GPIO.LOW)
            else:
                GPIO.output(solenoidR_pin, GPIO.HIGH)
                sleep(0.1)
